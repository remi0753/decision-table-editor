  ...
  return (
    <div className="flex-1 min-w-0 flex items-end self-stretch pl-3 pt-1">
      <div className="relative min-w-0 self-stretch flex items-end">
        {canScrollLeft && (
          <button
            ...
            className="absolute left-1 top-1/2 -translate-y-1/2 z-20 inline-flex items-center justify-center w-6 h-6 rounded bg-surface/95 border border-line shadow-sm text-fg-muted transition-colors hover:text-brand-fg hover:bg-brand-subtle"
          >
        ...
        {canScrollRight && (
          <button
            ...
            className="absolute right-1 top-1/2 -translate-y-1/2 z-20 inline-flex items-center justify-center w-6 h-6 rounded bg-surface/95 border border-line shadow-sm text-fg-muted transition-colors hover:text-brand-fg hover:bg-brand-subtle"
          >
          ...
          {tables.map((table) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'group relative flex items-center gap-1.5 pl-4 pr-2 text-xs cursor-pointer shrink-0 max-w-[220px] select-none border transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-brand-ring',
                  isActive
                    ? 'h-[39px] -mb-px pb-[7px] rounded-t-md border-0 border-b-[3px] border-b-violet-600 text-brand-fg font-semibold z-10'
                    : 'h-8 mb-1.5 rounded-md border-transparent bg-transparent text-fg-subtle hover:border-line hover:bg-surface-muted hover:text-fg-secondary',
                )}
              >
                {isEntry && (
                  <LogIn
                    ...
                    className={cn(
                      'shrink-0',
                      isActive ? 'text-brand-fg' : 'text-brand-fg-mid',
                    )}
                ...
                {isRenaming ? (
                  <input
                    ...
                    className="min-w-0 w-[132px] bg-surface border border-brand-border-strong rounded px-1.5 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-brand-ring"
                  />
                ) : (
                ...
                <button
                  ...
                  className={cn(
                    'shrink-0 inline-flex items-center justify-center w-4 h-4 rounded text-fg-faint transition-colors hover:text-danger-fg hover:bg-danger-bg',
                    isActive
                      ? 'opacity-100 text-brand-fg-mid'
      ...
      <button
        ...
        className="shrink-0 ml-1 mb-2 inline-flex items-center justify-center w-7 h-7 rounded text-fg-faint transition-colors hover:text-brand-fg hover:bg-brand-subtle"
      >
